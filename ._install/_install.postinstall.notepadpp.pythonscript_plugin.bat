...
call "%%CONTOOLS_ROOT%%/build/mkdir.bat" "%%PYTHON_EXTRACT_TEMP_DIR%%"
...
call "%%CONTOOLS_ROOT%%/build/call.bat" "%%CONTOOLS_BUILD_TOOLS_ROOT%%/extract_files_from_archive.bat" ^
  ...
  echo.
  call "%%CONTOOLS_ROOT%%/build/xmove_file.bat" "%%PYTHON_EXTRACT_TEMP_DIR%%\%%PYTHON_PACKAGE_DIR_NAME%%\" "*.*" "%%DETECTED_NPP_PYTHONSCRIPT_PLUGIN_ROOT%%\" /E /Y || (
...
call "%%CONTOOLS_ROOT%%/build/mkdir.bat" "%%PSUTIL_EXTRACT_TEMP_DIR%%"
...
call "%%CONTOOLS_ROOT%%/build/call.bat" "%%CONTOOLS_BUILD_TOOLS_ROOT%%/extract_files_from_archive.bat" ^
  ...
  echo.
  call "%%CONTOOLS_ROOT%%/build/xmove_file.bat" "%%PSUTIL_EXTRACT_TEMP_DIR%%\%%PSUTIL_PACKAGE_DIR_NAME%%\Lib\site-packages\" "*.*" "%%DETECTED_NPP_PYTHONSCRIPT_PLUGIN_ROOT%%\lib\site-packages\" /E /Y || (
...
call "%%CONTOOLS_ROOT%%/build/xcopy_file.bat" "%%DETECTED_NPP_INSTALL_DIR%%/plugins/PythonScript" python27.dll "%%DETECTED_NPP_INSTALL_DIR%%" /Y /D /H
...
call "%%CONTOOLS_ROOT%%/build/mkdir.bat" "%%CTYPES_EXTRACT_TEMP_DIR%%"

rem if not exist "\\?\%DETECTED_NPP_PYTHONSCRIPT_PLUGIN_ROOT%\dlls\*" call "%%CONTOOLS_ROOT%%/build/mkdir.bat" "%DETECTED_NPP_PYTHONSCRIPT_PLUGIN_ROOT%\dlls"
...
call "%%CONTOOLS_ROOT%%/build/call.bat" "%%CONTOOLS_BUILD_TOOLS_ROOT%%/extract_files_from_archive.bat" ^
  ...
  echo.
  call "%%CONTOOLS_ROOT%%/build/xmove_file.bat" "%%CTYPES_EXTRACT_TEMP_DIR%%\ctypes-python-2.7.18\DLLs\" "_ctypes.pyd" "%%DETECTED_NPP_PYTHONSCRIPT_PLUGIN_ROOT%%\lib\" /E /Y || (
